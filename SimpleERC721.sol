...
contract SimpleERC721 is ERC721URIStorage {
    ...
    event Mint(address indexed _from, uint256 royalty, uint256 tokenID);
    event List(address indexed _from, uint256 royalty, uint256 price, uint256 tokenID);
    event Delist(address indexed _from, uint256 _tokenId);
    event Purchase(address indexed _from, address indexed _to, uint256 tokenID, uint256 price);
    ...
    function newItem(string memory tokenURI, uint256 royalty) public returns (uint256) {
        ...
        emit Mint(tmp.publisher, royalty, newItemId);
        return newItemId;
    }
    ...
    function listOnMarketplace(uint256 _tokenId, uint256 _price) public {
        ...
        emit List(marketplace[_tokenId].publisher, marketplace[_tokenId].royalty, _price, _tokenId);
    }
    ...
    function removeFromMarketplace(uint256 _tokenId) public {
        ...
        emit Delist(marketplace[_tokenId].publisher, _tokenId);
    }
    ...
    function purchase(uint256 _tokenId) public payable {
        ...
        marketplace[_tokenId].listing = false;
        marketplace[_tokenId].price = MAX_INT;
        emit Purchase(ownerOf(_tokenId), marketplace[_tokenId].publisher, _tokenId, marketplace[_tokenId].price);
    }
}
